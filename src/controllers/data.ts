  ...
  async getFunnelChartData(ctx: ParameterizedContext) {
    ...
    const tmp = data.map((element) => {
      return {
        ...
        date: element.date,
        funnelChart: [
          { label: `${element.spm.d}点位`, count: element.total },
          { label: `${element.spm.c}区块`, count: element.level3[0].total },
          { label: `${element.spm.b}页面`, count: element.level2[0].total },
          { label: `${element.spm.a}站点`, count: element.level1[0].total },
        ],
      };
    });
